export function initCountryFilter() {
  const countrySearchInput = document.querySelector(".country-search-box__input");
  const checkboxGroups = document.querySelectorAll(".checkbox-group, .checkbox-group-special");
  ...
  if (countrySearchInput && checkboxGroups.length > 0) {
    ...
    countrySearchInput.addEventListener("input", () => {
      ...
      checkboxGroups.forEach(group => {
        ...
        const navContainer = document.querySelector(`.pagination-nav[data-nav-id="${navId}"]`);
  ...
  function initGroupPagination(group) {
    const checkboxes = group.querySelectorAll(".container-checkbox");
    const itemsPerPage = group.classList.contains('checkbox-group-special') ? 6 : 12;
